...
export async function GET() {
  try {
    ...
    if (!site) {
      console.error(`[Sitemap] No site found`);
      return new NextResponse(
        `<?xml version="1.0" encoding="UTF-8"?><error>No site found</error>`,
        {
          status: 404,
    ...
    console.log(`[Sitemap] Generating for site:`, {
      siteId: site.id,
      domain: site.domain,
      baseUrl,
    });
    ...
    console.log(`[Sitemap] Found ${products.length} products, ${categories.length} categories`);
