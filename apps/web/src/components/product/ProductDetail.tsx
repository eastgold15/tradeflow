...
const getActualMediaType = (media: {
  ...
  if (VIDEO_EXTENSIONS.some((ext) => media.url?.toLowerCase().endsWith(ext)))
    return "video";
...
const ProductDetail: React.FC<ProductDetailProps> = ({ siteProduct }) => {
  ...
  useEffect(() => {
    if (!isInitialized.current && skus.length > 0) {
      if (parsedSizeData) {
        setActiveSizeRegion(
          Object.keys(parsedSizeData.regionGroups)[0] || "EU"
        );
      }
    ...
    return skus.find((sku) => {
      ...
      return Object.entries(selectedSpecs).every(([key, value]) => {
        if (key === parsedSizeData?.sizeKey)
          return specJson[key].includes(value);
      ...
      <div className="mx-auto max-w-325 px-6">
        <div className="mb-24 grid grid-cols-1 gap-12 lg:grid-cols-12">
          ...
          <div className="pt-8 pl-4 lg:col-span-5">
              ...
              <div className="mb-6">
                ...
                <div className="relative w-full">
                  <select
                    ...
                    onChange={(e) =>
                      handleSpecChange(parsedSizeData.sizeKey, e.target.value)
                    }
                  ...
                  >
                    ...
                    {(parsedSizeData.regionGroups[activeSizeRegion] || []).map(
                      (sizeNum) => (
                        <option
                          key={sizeNum}
                          value={`${activeSizeRegion}${sizeNum}`}
                        >
                          {sizeNum}
                        </option>
                      )
            ...
            <div className="mb-8 space-y-6">
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-[10px] text-gray-500 tracking-widest" />
                </div>
                <div className="flex items-center gap-2 border border-gray-200 px-2 py-1">
                  ...
                  <input
                    className="min-w-20 flex-1 border-gray-200 border-x px-3 py-2 text-center font-medium text-sm focus:border-2 focus:border-black focus:ring-2 focus:ring-black/5"
                    min="1"
                    onChange={(e) => {
            ...
            <p className="text-center font-bold text-gray-500 text-lg tracking-widest">
              Please choose QTY & Payment term
            </p>
            <p className="text-center text-gray-500 text-sm tracking-widest">
              push below button for{" "}
              <b className="text font-bold text-lg">
                {" "}
                discount wholesale offer
              </b>
            </p>
          ...
          <div className="font-serif text-gray-800 text-lg leading-relaxed">
            {activeTab === "description" ? (
              ...
              <div className="space-y-6">
                <p>
                  <span className="mr-4 font-serif uppercase">Categories</span>
